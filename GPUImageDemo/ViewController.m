...
- (void)setUpFilter{
    ...
    //2、创建滤镜对象     可以创建不同的滤镜对象
//    _filter = [[GPUImageSobelEdgeDetectionFilter alloc]init];
    _filter = [[GPUImageGaussianBlurFilter alloc]init];
    
    //----------------------设置滤镜相关属性-----------------------
//    [(GPUImageTiltShiftFilter *)_filter setBlurRadiusInPixels:10];
//    [(GPUImageTiltShiftFilter *)_filter setFocusFallOffRate:15];
    
    CGRect mainScreenFrame = [[UIScreen mainScreen] applicationFrame];
    GPUImageView *primaryView = [[GPUImageView alloc] initWithFrame:mainScreenFrame];
    self.view = primaryView;
#pragma warning - 有坑！！！！ 要按照这个写法设置
    GPUImageView *targetView = (GPUImageView *)self.view;
    
    [_filter forceProcessingAtSize:targetView.sizeInPixels];
    
    //3、给资源图片添加滤镜
    [_sourcePicture addTarget:_filter];
    //4、给滤镜添加目标
    [_filter addTarget:targetView];
    //5、开始操作
